...
def main(args):
    ...
    # set projection head from uda config to model config
    cfg["model"]["decode_head"]["projection_head"] = (
        cfg["uda"]["enable_contrastive"]
        if "enable_contrastive" in cfg["uda"]
        else False
    )
    ...
    if args.seed is not None:
        ...
        logger.info(
            f"Set random seed to {args.seed}, deterministic: " f"{deterministic}"
        )
