...
class PixelContrastMemoryBankLoss(MemoryBankModule):
    def __init__(self, temperature=0.1, base_temperature=0.1, ignore_label=-1, max_samples=256*256*15, max_views=8, num_classes=15, memory_bank_size=512):
        super().__init__(num_classes=num_classes, size=memory_bank_size)
        ...
        for ii in range(batch_size):
            ...
            if len(this_classes):
                ...
                for cls_id in this_classes:
                    if weight is not None:
                        hard_indices = ((this_y_hat == cls_id) & (this_y != cls_id) & (this_w > 0)).nonzero()
                        easy_indices = ((this_y_hat == cls_id) & (this_y == cls_id) & (this_w > 0)).nonzero()
    ...
    def forward(self, feats, labels, predict, weight=None):
        ...
        queue = super(PixelContrastMemoryBankLoss, self).forward(
                feats.detach().clone(), labels, weight=weight, update=True
            )
...
class ContrastMemoryBankMixCELoss(nn.Module):
    """CrossEntropyLoss.
    ...
    def __init__(self,
        ...
        super(ContrastMemoryBankMixCELoss, self).__init__()
        ...
        self.pixel_contrast = PixelContrastMemoryBankLoss(
            ...
            ignore_label=ignore_label, max_samples=max_samples, max_views=max_views,
            num_classes=num_classes, memory_bank_size=memory_bank_size)
